...
Run `skill-issue install` in an interactive terminal for guided installation. Use the up and down arrow keys to select Claude Code, OpenAI Codex, Cursor, OpenCode, Kilo Code, or Pi and then select project or user scope. Project scope uses the current working directory. Before writing anything, the CLI previews the harness, scope, native destination, and eleven embedded skills and asks for confirmation. It does not detect or automatically choose a harness.
