...
This campaign used only embedded and custom two-turn smoke inputs. It did not run a governed 30-turn evaluation or implementation audit. Every evaluated project used a clean temporary Git workspace. The Kilo addition was followed separately by the full Go test suite and repository formatting validation.
...
| Kilo Code   | `7.4.11`; native OpenAI ChatGPT OAuth                                   | Built-in default and explicit `openai/gpt-5.6-sol` custom routes complete |
...
- Kilo uses run-owned configuration, supplied skills, state, cache, and temporary roots while retaining its operator-owned OAuth data root. It disables ambient project configuration, external skills, indexing, remote control, daemon use, and unrelated tools.
- Kilo's adjacent byte-identical JSON event pairs are collapsed before validation and scoring. Session cleanup deletes the captured ID directly and does not call Kilo `7.4.11`'s broken global-list route.
...
- Model and reasoning: `openai/gpt-5.6-sol`, `medium`
- The expected `document-update-discipline` call was observed on Turn 1; additional `system-change-ownership` remained valid model behavior.
- Both turns completed through one stable native session with no missing expected calls.
...
| Kilo Code   | `openai/gpt-5.6-sol`, `medium`       | `smoke-skill` on both turns                                      |
...
- Kilo runs removed supplied skills and private runtime state, deleted both exact native session IDs, left the daemon stopped, and left no Kilo process.
- Kilo transcript captures contained one stable session ID per run and zero adjacent exact duplicates in their normalized structured event arrays.
