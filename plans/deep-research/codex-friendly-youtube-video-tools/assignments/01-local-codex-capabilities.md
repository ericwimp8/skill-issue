...
- Local research framing: `plans/deep-research/codex-friendly-youtube-video-tools/research-map.md`.
- Local Codex configuration: `~/.codex/config.toml`, especially enabled `computer-use`, `chrome`, and `browser` plugins (lines 56-78) and Browser/Computer Use runtime settings (lines 256-281).
- Local image skill: `<local-codex-config>/skills/.system/imagegen/SKILL.md`, lines 1-60; it defines an `image_gen` workflow for raster-image generation and editing.
- Local Browser skill: `~/.codex/plugins/cache/openai-bundled/browser/26.715.31925/skills/control-in-app-browser/SKILL.md`, lines 1-50; Browser is for navigation, interaction, screenshots, and local-web testing.
- Local Computer Use skill: `~/.codex/plugins/cache/openai-bundled/computer-use/1.0.1000451/skills/computer-use/SKILL.md`, lines 1-40; it controls local Mac app UI.
- Local plugin manifests: `~/.codex/plugins/cache/openai-bundled/{browser/26.715.31925,computer-use/1.0.1000451,chrome/26.715.31925}/.codex-plugin/plugin.json`.
- Local command probes, run 2026-07-19: `codex --version`; `command -v ffmpeg ffprobe ffplay yt-dlp obs screencapture`; `ffmpeg -hide_banner -filters`; `ffmpeg -hide_banner -encoders`; `screencapture -h`; `find /Applications ...`; and inventory searches under `~/.codex/skills` and `~/.codex/plugins/cache`.
...
`ffmpeg` 8.1.1, `ffprobe`, and `ffplay` are installed at `/opt/homebrew/bin`. The FFmpeg probes confirmed `libx264`, `h264_videotoolbox`, `hevc_videotoolbox`, `aac`, and `aac_at` encoders, plus `concat`, `xfade`, `overlay`, `scale`, `fps`, `amix`, and `loudnorm` filters. This is a usable local, scriptable path for stitching, transitions, title/overlay compositing, format conversion, 1080p export, audio mixing, and loudness normalization.

macOS `/usr/sbin/screencapture` is installed. Its built-in usage reports `-v` video recording, `-V<seconds>` duration limiting, `-g`/`-G` audio capture, `-k` click visualization, interactive `-J video` selection, display selection, and file output. This supplies a native recording path without another recorder installation.

**Evidence:** Local probes returned `ffmpeg version 8.1.1`, the listed encoders/filters, and `screencapture` usage containing the above video options. `yt-dlp` and `obs` were unavailable. A top-level `/Applications` scan returned no matching installed iMovie, Final Cut Pro, ScreenFlow, CapCut, DaVinci Resolve, Premiere, Camtasia, or OBS application.
...
**Evidence:** `~/.codex/config.toml` lines 56-60 and 77-78 enable the three plugins. The local Browser and Computer Use skill descriptions name navigation/testing/screenshots and Mac-app UI control respectively. The official Computer Use guide describes GUI operation rather than media-composition functions.
...
- The local `computer-use` MCP server entry is `enabled = false` in `~/.codex/config.toml`, even though the `computer-use` plugin itself is enabled and the capability is surfaced in this session. Actual GUI operation still depends on the active desktop-app/runtime connection, macOS permissions, and per-app approval.
