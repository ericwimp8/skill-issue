...
- Local probe: `/usr/sbin/screencapture` on macOS 26.2 (25C56); its usage documents `-v` video capture, `-g` default-input audio, `-G<id>` audio-device capture, `-D<display>`, `-R<x,y,w,h>`, `-V<seconds>`, `-k` click highlighting, and `-U` interactive toolbar.
- Local probe: `/opt/homebrew/bin/ffmpeg` 8.1.1, Homebrew formula currently labelled GPL-3.0-or-later. Its build enables `libx264`, VideoToolbox, and AudioToolbox; it exposes `concat`, `xfade`, `fade`, `overlay`, `amix`, `sidechaincompress`, `loudnorm`, `scale`, `fps`, `h264_videotoolbox`, `libx264`, `aac`, and `aac_at`.
...
- Local probe: no OBS Studio app was found in `/Applications`, `~/Applications`, or Spotlight. A TCC database entry grants an old `com.obsproject.obs-studio` microphone and camera permission, but it does not establish that OBS is installed or can screen-capture now.
