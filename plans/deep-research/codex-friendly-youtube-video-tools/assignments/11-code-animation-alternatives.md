...
- Local non-destructive probe, 2026-07-19: Node `v22.22.0` and FFmpeg `8.1.1` resolve; `editly`, `motion-canvas`, and `manim` do not resolve on `PATH`; `/Applications/Keynote.app` is absent.
...
- **Caveat:** Keynote was not found at `/Applications/Keynote.app` and Apple’s inspected media page did not establish a scriptable export interface, so it is only a manual-comparison option.
