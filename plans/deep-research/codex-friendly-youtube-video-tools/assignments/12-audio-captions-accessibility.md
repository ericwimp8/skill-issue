...
- Local capability probe, 2026-07-19: `/opt/homebrew/bin/ffmpeg` is version `8.1.1`; it exposes `anlmdn` and `loudnorm`, but this build exposes neither `subtitles` nor `whisper` filters and its build configuration has no `--enable-libass`. `whisper-cli`, `whisper`, and Audacity are absent.
