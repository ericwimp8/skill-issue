...
The project has a transparent and repeatable skill-calling evaluation in which the CLI generates a temporary installation from the current canonical skills, injects one opaque marker instruction without changing selection metadata, and runs three fixed 30-turn scenarios against each harness's primary agent. The runner sends the same substantial user turns regardless of model responses, records observed skill activations against private turn state, compares them with a separately supplied answer sheet, and emits structured data that can be plotted directly. A tooling-complete suite is valid whether it records every expected call or no calls; the published output describes observed behavior without pass or fail labels.
...
- Give each scenario a separate conversation document, runner instructions, and answer sheet.
...
- Define the exact evaluation instruction the CLI injects at the beginning of the executable workflow and require it to record the invocation before continuing the unchanged canonical body.
...
- Use the CLI's evaluation-install mode to prepare the target harness with generated evaluator copies of the supplied Skill Issue and supporting skills plus the shared marker asset.
...
- Keep the answer sheet, token map, and turn state outside the evaluated workspace and hidden from the evaluated primary agent.
...
- Emit a portable JSON result containing the run metadata, scenario, turn count, expected first-activation turns, observed skill events, missing expected events, and any additional observed calls.
...
- Make the JSON shape directly consumable by the website's benchmark data and graphing work without manual reinterpretation of the conversation transcript.
...
- Three governed 30-turn scenario sets are stored as inspectable assets, each containing the fixed conversation, runner instructions, and separately supplied expected-call answer sheet.
...
- Every completed scenario suite emits a portable JSON evidence package containing run metadata, expected calls, observed calls, missing expected calls, additional calls, and the source evidence needed for audit.
...
- The website can consume the structured results and display descriptive comparisons without pass or fail labels.
...
- The gardening web-application scenario and common Go runner are complete. The other two scenarios, live cross-harness proof, and campaign aggregation remain open.
...
`evaluations/skill-calling/instrumentation-contract.md` and `evaluations/skill-calling/event.schema.json` now own the generated-copy transformation, opaque `mark` command, private state, turn attribution, marker behavior, cleanup, and portable invocation-event shape.

## Completed First Scenario

The gardening scenario contains 30 fixed substantial turns, direct-runner instructions, and a separately supplied answer sheet defining required first activations and later applicable turns. Dictate Plan is installed from the current canonical payload rather than a maintained instrumented variant.
