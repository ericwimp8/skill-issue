...
- `cli/internal/payload/assets/manifest.json` contains the complete ordinary and evaluation-only component inventory, and the payload owner validates canonical frontmatter and referenced-file closure.
...
- All three governed scenarios, their embedded private answer sheets, and the evaluation-only Dictate Plan source are complete. Paired external scenario and answer-sheet JSON files remain supported for custom evaluations.
