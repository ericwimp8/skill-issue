...
| 1:16–1:43 | “The open-source CLI includes three compact multi-turn evaluations and can run custom skills, scenarios, and answer sheets through the same pipeline. It writes detailed local evidence and compact chart-ready results.”   | Show a built-in evaluation command, the custom-input route, and the generated `result.json` and `website.json`.   |
