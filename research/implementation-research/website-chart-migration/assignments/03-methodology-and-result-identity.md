...
**Evidence:** The supporting plan separates invocation measurement from body-quality grading and requires expected first-activation turns to be recorded before results are observed. The human-readable answer sheet names four required first activations and says later applicable turns do not require another event when the harness keeps a skill loaded. It classifies unmatched required activations as missing and other turn/skill events as additional, without a pass/fail verdict.
...
Each governed built-in is a schema-version-1 unit containing an `evaluation_id`, one scenario with `scenario_id` and an ordered array of `{turn_id, prompt}`, and one private answer sheet with the same `scenario_id` and an `expected` array of `{turn_id, skill}`. The three current built-ins each have 30 turns and the same four expected first-activation pairs: turn 1 for `document-update-discipline`, turn 11 for `prompt-writing`, turn 25 for `skill-authoring-discipline`, and turn 30 for `system-change-ownership`.

**Evidence:** The gardening, community archive, and emergency-preparedness built-ins carry the same four expected pairs in their answer-sheet sections. Production validation requires schema version 1, a nonempty scenario ID, at least one turn, unique nonempty turn IDs, answer-sheet/scenario identity agreement, existing turn IDs, and available skill names.
...
The MVP requires one tooling-complete suite of all three governed scenarios for every one of the thirteen cells. It does not require repeated trials, statistical thresholds, or a pass threshold. This implies 39 atomic scenario runs and 12 expected first-activation observations per cell under the current fixtures, but only one suite-level observation per cell.
