...
| Kilo 7.4.11 | `openai/gpt-5.6-sol`, medium | Clean controlled environment, private XDG config/state/cache/temp, pure code agent, supplied skill path only, project/external skills/indexing/memory/daemon disabled, deny-first permissions | Retains real `HOME` and operator authentication data root; compiled internal OAuth remains |
