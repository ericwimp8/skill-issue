...
This is the initiating prompt used to run the complete Skill Issue intake, generation, and evaluation workflow for `accessibility-first-pass`. The machine-specific checkout path has been normalized to `<repo-root>` for publication.
